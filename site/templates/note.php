...
<article class="note">
  <header class="note-header h1">
    <h1 class="note-title"><?= $page->title()->escape() ?></h1>
    <?php if ($page->subheading()->isNotEmpty()): ?>
    <p class="note-subheading"><small><?= $page->subheading()->escape() ?></small></p>
    <?php endif ?>
  </header>
  <div class="note text">
    <?= $page->text()->toBlocks() ?>
  ...
  <footer class="note-footer">
    ...
    <time class="note-date" datetime="<?= esc($page->date('c'), 'attr') ?>">Published on <?= esc($page->date()) ?></time>
  </footer>
